...
var Preprocessor = require('../tokenizer/preprocessor'),
    LocationInfoMixin = require('./location_info_mixin'),
    UNICODE = require('../utils/unicode'),
    NAMED_ENTITY_TRIE = require('../tokenizer/named-entity-trie');


//Aliases
var $ = UNICODE.CODE_POINT,
    $$ = UNICODE.CODE_POINT_SEQUENCE;
...
var isWhitespace = require('../utils/unicode').isWhitespace;
var isAsciiDigit = require('../utils/unicode').isAsciiDigit;
var isAsciiUpper = require('../utils/unicode').isAsciiUpper;
var isAsciiLower = require('../utils/unicode').isAsciiLower;
var isDigit = require('../utils/unicode').isDigit;
var isAsciiAlphaNumeric = require('../utils/unicode').isAsciiAlphaNumeric;
var isReservedCodePoint = require('../utils/unicode').isReservedCodePoint;
var toAsciiLowerCodePoint = require('../utils/unicode').toAsciiLowerCodePoint;
var toChar = require('../utils/unicode').toChar;
var toAsciiLowerChar = require('../utils/unicode').toAsciiLowerChar;
...
Tokenizer.prototype.isTempBufferEqualToScriptString = function () {
    if (this.tempBuff.length !== $$.scriptString.length)
        return false;

    for (var i = 0; i < this.tempBuff.length; i++) {
        if (this.tempBuff[i] !== $$.scriptString[i])
            return false;
    }
    ...
    if (referencedCodePoints) {
        if (!semicolonTerminated) {
            ...
            if (inAttr) {
                ...
                if (nextCp === $.eqSign || isAsciiAlphaNumeric(nextCp)) {
...
Tokenizer.prototype._consumeCharacterReference = function (startCp, inAttr) {
    if (this.disableEntitiesDecoding || isWhitespace(startCp) || startCp === $.greaterThanSign ||
...
_[SCRIPT_DATA_DOUBLE_ESCAPE_START_STATE] = function scriptDataDoubleEscapeStartState(cp) {
    if (isWhitespace(cp) || cp === $.solidus || cp === $.greaterThanSign) {
...
_[BEFORE_ATTRIBUTE_NAME_STATE] = function beforeAttributeNameState(cp) {
    ...
    else if (cp === $.quotationMark || cp === $.apostrophe || cp === $.lessThanSign || cp === $.eqSign) {
...
_[ATTRIBUTE_NAME_STATE] = function attributeNameState(cp) {
    ...
    else if (cp === $.quotationMark || cp === $.apostrophe || cp === $.lessThanSign)
...
_[BEFORE_ATTRIBUTE_VALUE_STATE] = function beforeAttributeValueState(cp) {
    ...
    else if (cp === $.lessThanSign || cp === $.eqSign || cp === $.graveAccent) {
...
_[ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE] = function attributeValueDoubleQuotedState(cp) {
    if (cp === $.quotationMark)
        ...
        this.additionalAllowedCp = $.quotationMark;
...
_[ATTRIBUTE_VALUE_UNQUOTED_STATE] = function attributeValueUnquotedState(cp) {
    if (isWhitespace(cp))
        ...
        this.additionalAllowedCp = $.greaterThanSign;
        this.returnState = this.state;
        this.state = CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE;
    }
...
_[MARKUP_DECLARATION_OPEN_STATE] = function markupDeclarationOpenState(cp) {
    if (this._consumeSubsequentIfMatch($$.dashDashString, cp, true)) {
        this._createCommentToken();
        this.state = COMMENT_START_STATE;
    }

    else if (this._consumeSubsequentIfMatch($$.doctypeString, cp, false))
        this.state = DOCTYPE_STATE;

    else if (this.allowCDATA && this._consumeSubsequentIfMatch($$.CDATAStartString, cp, true))
...
_[AFTER_DOCTYPE_NAME_STATE] = function afterDoctypeNameState(cp) {
    ...
    else if (this._consumeSubsequentIfMatch($$.publicString, cp, false))
        this.state = AFTER_DOCTYPE_PUBLIC_KEYWORD_STATE;

    else if (this._consumeSubsequentIfMatch($$.systemString, cp, false))
...
_[CDATA_SECTION_STATE] = function cdataSectionState(cp) {
    while (true) {
        ...
        else if (this._consumeSubsequentIfMatch($$.CDATAEndString, cp, true)) {
            this.state = DATA_STATE;
            break;
        }
